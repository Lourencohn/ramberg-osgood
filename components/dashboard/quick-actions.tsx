        ...
        {actions.map((action, index) => (
          ...
          >
            <div
              ...
              ${index === 0 ? 'bg-background/20' : 'bg-muted group-hover:bg-foreground/10'}
            `}
            >
              <action.icon
                className={`size-5 ${index === 0 ? 'text-background' : 'text-foreground'}`}
              />
            </div>
            <div className="flex-1 min-w-0">
              <p
                className={`text-sm font-semibold ${index === 0 ? 'text-background' : 'text-foreground'}`}
              >
                {action.title}
              </p>
              <p
                className={`text-xs mt-0.5 ${index === 0 ? 'text-background/70' : 'text-muted-foreground'}`}
              >
                {action.description}
              </p>
            </div>
            <ArrowRight
              className={`size-4 transition-transform group-hover:translate-x-1 ${index === 0 ? 'text-background/70' : 'text-muted-foreground'}`}
            />
          </Link>
        ))}
      </CardContent>
